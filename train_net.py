        ...
        * Pixel Accuracy (pACC)
        ...
        for i, name in enumerate(self._class_names):
            ...
            if name in self._val_extra_classes:
                ...
                res_unseen["IoU-{}".format(name)] = 100 * iou[i]
            else:
                seen_IoU = seen_IoU + 100 * iou[i]
        assert len(res_unseen) == len(self._val_extra_classes)
        ...
        print('len unseen results:', len(res_unseen))
        print("unseen results: ", res_unseen)
        return results
    ...
    def build_optimizer(cls, cfg, model):
        ...
        for module_name, module in model.named_modules():
            for module_param_name, value in module.named_parameters(recurse=False):
                ...
                if ("semantic_query" in module_name):
                    # import ipdb; ipdb.set_trace()
                    hyperparams["lr"] = hyperparams["lr"] * cfg.MODEL.MASK_FORMER.SEMANTIC_QUERY_MULTIPLIER
                # if ("bg_feature" in module_name):
                #     import ipdb;
                #     ipdb.set_trace()
                if (
