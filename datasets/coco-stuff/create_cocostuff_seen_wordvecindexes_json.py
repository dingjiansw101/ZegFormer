...
for item in COCO_CATEGORIES:
    ...
    if id in train_cls:
        seen_classnames.append(id)
        tmp = copy.deepcopy(item)
        tmp['trainId'] = count_train
        COCO_CATEGORIES_Seen.append(tmp)
        count_train = count_train + 1
    else:
        unseen_classnames.append(id)
        tmp = copy.deepcopy(item)
        tmp['trainId'] = count_test
        COCO_CATEGORIES_unseen.append(tmp)
        count_test = count_test + 1

with open(r'datasets/coco/coco_stuff/split/seen_indexes.json', 'w') as f_out:
    json.dump(seen_classnames, f_out)

with open(r'datasets/coco/coco_stuff/split/unseen_indexes.json', 'w') as f_out:
    json.dump(unseen_classnames, f_out)

with open(r'datasets/coco/coco_stuff/split/all_indexes.json', 'w') as f_out:
