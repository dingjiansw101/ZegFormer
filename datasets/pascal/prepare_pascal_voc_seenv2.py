...
import os
# This code is for pascal voc v2, that only has 10582 images
...
if __name__ == '__main__':
    ...
    train_list = np.load(r'datasets/VOCZERO/split/train_list.npy')
    ...
    for name in ["val", "train"]:
        ...
        output_image_dir = os.path.join(dataset_dir, "images_detectron2", f"{name}_seen")
        Path(output_image_dir).mkdir(parents=True, exist_ok=True)

        annotation_dir = dataset_dir / "annotations" / name
        output_dir = dataset_dir / "annotations_detectron2_v2" / f"{name}_seen"
        ...
        for file in tqdm.tqdm(list(annotation_dir.iterdir())):
            ...
            copyfile(
                ...
                os.path.join(output_image_dir, basename + ".jpg"),
            )
